function init() {
    ...
    function stabilizeCraftrasBody(body, previousHealth = null) {
        if (!Config.craftras || !body || body.isDead?.()) return;
        const getLevelHealthBonus = level => {
            let remaining = Math.max(0, Math.trunc(Number(level) || 0));
            let bonus = 0;
            let amountPerLevel = 1;
            while (remaining > 0) {
                const levels = Math.min(100, remaining);
        ...
        const oldMax = previousHealth?.max > 0 ? previousHealth.max : body.health.max;
        const oldAmount = Number.isFinite(previousHealth?.amount) ? previousHealth.amount : body.health.amount;
        const ratio = oldMax > 0 ? util.clamp(oldAmount / oldMax, 0, 1) : 1;
        const restoredAmount = Math.max(1, desiredHealth * ratio);
        body.HEALTH = 100;
        body.SHIELD = 0;
        body.REGEN = 0;
        body.SPEED = 6;
        body.health.set(desiredHealth);
        body.health.amount = Math.min(desiredHealth, restoredAmount);
        body.shield.set(0, 0);
        body.damageReceived = 0;
        body.topSpeed = global.gameManager.runSpeed * 6 * (body.craftrasCreativeFlight ? 10 : 1);
    }
    ...
    global.runKeyCommand = (socket, keyCodes) => {
        ...
        const now = Date.now();
        const deny = message => {
            if (now >= (socket.craftrasNextKeyCommandDeniedAt || 0)) {
                socket.craftrasNextKeyCommandDeniedAt = now + 1500;
                socket.talk?.("m", 3_000, message);
            }
            return 1;
        };
        if (global.craftrasCheatsEnabled === false) return socket.permissions?.admin ? deny("Cheats are disabled.") : 1;
        ...
        if (lockedByTheSword && !socket.craftrasTheSwordOpOverride) return deny("A mysterious power prevents key commands.");
        ...
        if (command && (permsLevel >= command.level || (command.operatorAccess && socket.player.body.hasOperator))) {
          try {
            const previousHealth = Config.craftras && socket.player.body ? {
              amount: socket.player.body.health.amount,
              max: socket.player.body.health.max,
            } : null;
            command.run({
              socket,
            ...
            stabilizeCraftrasBody(socket.player.body, previousHealth);
