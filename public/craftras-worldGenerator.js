...
function loadManualCaves() {
    ...
    try {
        ...
        for (const key of data.caves || []) manualCaveSet.add(key);
    } catch {
...
function buildOpenDistanceMap(seed, options = {}) {
    ...
    const includeManualCaves = options.includeManualCaves !== false;
    const caves = includeManualCaves ? loadManualCaves() : null;
    let head = 0;
    let tail = 0;
